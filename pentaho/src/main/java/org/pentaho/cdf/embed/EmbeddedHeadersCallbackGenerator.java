...
public class EmbeddedHeadersCallbackGenerator extends EmbeddedHeadersGenerator {
  ...
  static final String DOCUMENT_SCRIPT_CALLBACK_CALL = "if({0}) {0}(require);";
  ...
  protected String printScriptsContext() {
    ...
    sb.append( MessageFormat.format( DOCUMENT_SCRIPT, fullQualifiedURL + REQUIRE_PATH ) )
      .append( MessageFormat.format( DOCUMENT_SCRIPT_CALLBACK, fullQualifiedURL + REQUIRE_START_PATH ) )
      .append( MessageFormat.format( DOCUMENT_SCRIPT, fullQualifiedURL + REQUIRE_INIT_PATH, callbackCall ) );
